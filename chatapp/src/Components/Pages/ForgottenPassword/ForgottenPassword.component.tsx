...
const ForgottenPassword: React.FunctionComponent<IForgottenPasswordProps> = (props) => {

  const [email, setEmail] = useState<string>('');
  const [emailSent, setEmailSent] = useState<boolean>(false);

  const onForgottenPasswordFormSubmitted = useCallback(async (e) => {
    e.preventDefault();
    try {
      await http.post('/sendPasswordRecoveryEmail', { email: email });
      setEmailSent(true);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="h-screen flex justify-center items-center bg-gray-50 duration-200 dark:bg-indigo-900" style={{ flexDirection: 'column' }}>
      <div className="m-5 text-2xl text-gray-900 dark:text-white">Chat App - Forgotten Password</div>
      {
        emailSent ?
          ...
          :
          <form className="text-black w-2/3 p-5 border bg-white rounded-lg border-inherit-600 md:w-1/4" onSubmit={onForgottenPasswordFormSubmitted}>
          ...
          Username or password is wrong
            ...
            <div className="mb-6">
              Enter the email address registered with your account. We'll email you the password reset link instantly.
            </div>
            <div className="mb-6">
              <input type="email" id="email" onChange={onInputChange} className="bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 dark:focus:ring-indigo-900 dark:focus:border-indigo-900" placeholder="Email" required />
            </div>
